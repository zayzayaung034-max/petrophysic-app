...
export const LasUploader: React.FC = () => {
  ...
  const handleDownloadCSV = async () => {
    ...
    try {
      ...
      console.warn('Backend CSV export unavailable, executing browser-side fallback:', err);
    }
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      alert(`Failed to download PDF report. Ensure backend is running on ${API_BASE_URL}.`);
    } finally {
